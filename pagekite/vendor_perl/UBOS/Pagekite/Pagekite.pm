...
sub activate {
    ...
    trace( 'Pagekite::activate', $namedKitesP, $allKites );

    # we rely on them being sorted
    $namedKitesP = [ sort @$namedKitesP ];

    my @activeKites = determineActiveKiteNames( $namedKitesP, $allKites );

    my $status = {
        'status'       => 'active',
        'enabledKites' => $namedKitesP,
        'activeKites'  => \@activeKites,
        'allKites'     => $allKites
    };
    updateStatus( $status );
...
##
# Determine whether there is a saved kite secret.
# return: true or false
sub hasKiteSecret {
...
sub determineActiveKiteNames {
    ...
    trace( 'Pagekite::determineActiveKiteNames', $namedKitesP, $allKites, \@ret );
...
sub deployedSitesUpdated {
    ...
    if( isActive() ) {
        ...
        generateKiteConfig( \@activeKites );
        activateDeactivateRestartSystemdService( \@activeKites, $status->{activeKites} );
