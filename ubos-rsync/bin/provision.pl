#!/usr/bin/perl
#
# Provision an entry in /home/${package.name}/.ssh/authorized_keys
#
...
if( 'deploy' eq $operation || 'undeploy' eq $operation ) {
    ...
    my $content = <<CONTENT;
# This file is automatically generated by ubos-rsync's provision.pl.
...
CONTENT
    ...
    foreach my $key ( sort keys %keyToAppConfigIds ) {
        ...
        $content .= 'command="/usr/share/ubos-rsync/bin/safe-rsync.pl '
                 . $packageName
                 . ' ' . join( ' ', @ids )
                 . '"';
        $content .= " $key\n";
